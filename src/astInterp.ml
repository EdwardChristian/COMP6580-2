...
let val_t_to_list (v : val_t) : val_t list =
    match v with
    | Vint _ -> raise TypeError
    | Varray _ -> raise TypeError
    | Vlist l -> l
...
let do_op op (n1 : int64) (n2 : int64) : int64 =
  ...
  | T.And | T.Or | T.Cons -> assert false
...
let rec interp_exp (env : env_t) (e : exp) : val_t =
  ...
  | Op (e1, T.Or, e2) ->
    (match interp_exp env e1 with
     ...
     | _ -> raise TypeError)
  | Op (e1, T.Cons, e2) ->
    let t1 = interp_exp env e1 in
    let t2 = interp_exp env e2 in 
    (match (t1, t2) with 
      | (t1, Vlist l) -> 
          Vlist (t1 :: l)
      | _ -> raise TypeError)
  ...
  | Uop (uop, e) ->
    (match interp_exp env e with
     ...
     | _ -> raise TypeError)
  | Lop (e, lop) -> 
    (match (interp_exp env e,lop) with
    | (Vlist(hd :: _),T.Head) -> hd
    | (Vlist(_ :: tl),T.Tail) -> (match tl with
                                  | [] -> Vlist([Vint(0L)])
                                  | _ -> Vlist(tl)
                                  )
    | (Vlist([]),T.Empty) -> Vint(1L)
    | (Vlist(hd :: tl), T.Empty) -> (match (hd,tl) with
                                    | (Vint(0L), []) -> Vint(1L)
                                    | (_,_) -> Vint(0L))
    | (_,_) -> raise TypeError)
  | Array iexps ->
  ...
  | List (hd, tl) ->
    Vlist ([interp_exp env hd] @ List.map (fun e -> (interp_exp env e)) tl)
...
(* 
  ...
  *
*)
(* Check if the function val_t_to_list correctly converts a Vlist variant containing 3 Vint variants into a list of Vint variants. *)
let test_val_t_to_list () =
  let lst = val_t_to_list (Vlist [Vint 1L; Vint 2L; Vint 3L]) in
  if lst = [Vint 1L; Vint 2L; Vint 3L] then
    ()
  else
    failwith "FAILED src/astInterp.ml::test_val_t_to_list"

(* Check if TypeError is raised when trying to convert an integer value to a list *)
let test_val_t_to_list_error () =
  try
    ignore (val_t_to_list (Vint 0L));
    failwith "FAILED src/astInterp.ml::test_val_t_to_list: TypeError not raised";
  with
    | TypeError -> ()

(* Check if creating two empty Vlist variants result in the same variant. *)
let test_vlist_creation_1 () =
  let v1 = Vlist [] in
  let v2 = Vlist [] in
  if v1 = v2 then
    ()
  else
    failwith "FAILED src/astInterp.ml::test_vlist_creation_1"

(* Check if creating two Vlist variants containing the same three Vint variants result in the same variant. *)
let test_vlist_creation_2 () =
  let v1 = Vlist [Vint 1L; Vint 2L; Vint 3L] in
  let v2 = Vlist [Vint 1L; Vint 2L; Vint 3L] in
  if v1 = v2 then
    ()
  else
    failwith "FAILED src/astInterp.ml::test_vlist_creation_2"

(* Check if creating two Vlist variants containing different numbers of Vint variants result in different variants. *)
let test_vlist_creation_3 () =
  let v1 = Vlist [Vint 1L; Vint 2L; Vint 3L] in
  let v2 = Vlist [Vint 1L; Vint 2L] in
  if v1 <> v2 then
    ()
  else
    failwith "FAILED src/astInterp.ml::test_vlist_creation_3"

(* Check if creating two Vlist variants containing different Vint variants result in different variants. *)
let test_vlist_creation_4 () =
  let v1 = Vlist [Vint 1L; Vint 2L; Vint 3L] in
  let v2 = Vlist [Vint 1L; Vint 2L; Vint 3L; Vint 4L] in
  if v1 <> v2 then
    ()
  else
    failwith "FAIL src/astInterp.ml::test_vlist_creation_4"

(* Run tests *)
let run_tests () =
  try
    test_val_t_to_list ();
    test_val_t_to_list_error ();
    test_vlist_creation_1 ();
    test_vlist_creation_2 ();
    test_vlist_creation_3 ();
    test_vlist_creation_4 ();
    print_endline "PASS src/astInterp.ml"
  with
