...
let source_typ_to_t (t : SourceAst.typ) : t =
  ...
  | List -> Tlist
(* Raise a type error *)
let raise_error (descr : string) (ln : int option) (msg : string) : 'a =
  match ln with
  | Some ln ->
    raise (BadInput (descr ^ " error on line " ^ string_of_int ln ^ ": " ^ msg))
  | None ->
    raise (BadInput (descr ^ " error at unknown location: " ^ msg))
...
let rec type_exp (ln : int option) (env : env_t) (e : exp) : t * exp =
  ...
  | Ident (i, es) ->
    let (t, scope) =
      ...
      with Not_found ->
        raise_error "binding" ln ("reference to undeclared variable " ^ show_id i)
    in
      ...
      (match t with
       | Tarray num_dims ->
         if num_dims = l && List.for_all (fun x -> x = Tint) ts then
           ...
           raise_error "type" ln "array index with non-integer type"
         else
           raise_error "arity" ln ("array reference with " ^ string_of_int l ^
                          " indices, expected " ^ string_of_int num_dims)
       | _ ->
         raise_error "type" ln ("attempt to index non-array variable " ^ show_id i))
  | Call (f, args) ->
    let (param_types, ret_type) =
      try Idmap.find f env.funs
      with Not_found ->
        raise_error "binding" ln ("call to undefined function " ^ show_id f)
    in
  ...
  | Op (e1, op, e2) ->
    ...
    let t =
      ...
      | (Tint, (T.Plus | T.Minus | T.Times | T.Div | T.Lshift | T.BitOr |
                T.BitAnd), Tint) ->
        Tint
      | (Tint, T.Cons, Tlist) -> Tlist (* you can cons an INT on the head of a Tlist *)
      | (Tint, (T.Lt | T.Eq | T.Gt), Tint) -> Tbool
      | (t1, _, t2) ->
        raise_error "type" ln ("operator " ^ T.show_op op ^
  ...
  | Uop (uop, e) ->
    ...
    (match (uop, t) with
     ...
     | (T.Not, t) ->
       raise_error "type" ln ("operator " ^ T.show_uop uop ^
                      " applied to expression of type " ^ show_t t))
  | Lop (e, lop) ->  (* LIST OPERATIONS TYPECHECKING *)
    let (t, e') = type_exp ln env e in
    (match (t, lop) with
      | (Tlist, T.Head) -> (Tint, Lop (e', lop))
      | (Tlist, T.Tail) -> (Tlist, Lop (e', lop))
      | (Tlist, T.Empty) -> (Tbool, Lop (e', lop))
      | (t, lop) -> raise_error "type" ln ("operator " ^ T.show_lop lop ^
                      " applied to expression of type " ^ show_t t))          
  | Array es ->
    let (ts, es') = List.split (List.map (type_exp ln env) es) in
    let l = List.length ts in
    if l = 0 then
      raise_error "semantic" ln "attempt to allocate array with 0 dimensions"
    else if List.for_all (fun x -> x = Tint) ts then
      (Tarray l, Array es')
    else
      raise_error "type" ln "array dimension with non-integer type"
  | List (hd,tl)-> (Tlist, List (hd,tl)) (* LIST TYPECHECKING - we accept any of the 4 types so there doesn't seem to be a need to typecheck, the operators deal with that *)
...
let rec type_stmt (ln : int option) (env :env_t) (return : t) (stmt : stmt)
  ...
  | In i ->
    ...
    if t <> Tint then
      raise_error "type" ln "input with non-integer type"
    else
      In i'
  | Out i ->
    let (t, i') = type_simple_ident ln env i in
    if t <> Tint then
      raise_error "type" ln "output with non-integer type"
    else
      Out i'
  | Return (Some i) ->
    let (t, i') = type_simple_ident ln env i in
    if t <> return then
      raise_error "type" ln ("return has type " ^ show_t t ^
                     " in a function with return type " ^
                     show_t return)
    else
      Return (Some i')
  | Return None ->
    raise_error "semantic" ln "return without value"
  | Assign (x, es, e) ->
    (match type_exp ln env (Ident (x, es)) with
     | (t1, Ident (x', es')) ->
       let (t2, e') = type_exp ln env e in
       if t1 <> t2 then
         raise_error "type" ln ("assignment to variable of type " ^ show_t t1 ^ " with type " ^
                        show_t t2)
       else
         Assign (x', es', e')
     | _ -> assert false)
  | DoWhile (s1, e, s2) ->
    ...
    (match type_exp ln env e with
     | (Tbool, e') ->
       ...
       DoWhile (s1', e', s2')
     | _ ->
      raise_error "type" ln "do/while test of non-bool type")
  | Ite (e, s1, s2) ->
    (match type_exp ln env e with
     | (Tbool, e') ->
       let s1' = type_stmt ln env return s1 in
       let s2' = type_stmt ln env return s2 in
       Ite (e', s1', s2')
     | _ -> raise_error "type" ln "if test of non-bool type")
  | Stmts (s_list) ->
...
let rec get_param_types (ln : int option) (params : (id * typ) list)
  ...
  | (x,t)::params ->
    if Idmap.mem x param_env then
      raise_error "semantic" ln ("duplicate function parameter " ^ show_id x)
    else
...
let rec get_var_types (s : scope) (vars : var_dec list)
  ...
  | v::vars ->
    if Idmap.mem v.var_name var_env then
      raise_error "semantic" v.loc ("duplicate variable definition " ^ show_id v.var_name)
    else
...
let type_var_dec (s : scope) (env : env_t) (dec : var_dec) : var_dec =
  ...
  if t = source_typ_to_t dec.typ then
    ...
    raise_error "type" dec.loc ("initialisation of variable typed " ^
...
let type_function (env : env_t) (f : func) : func =
  ...
  if not (check_return_paths f.body) then
    raise_error "semantic" f.loc "function might not return"
  else
    { f with
...
let rec get_function_types (funcs : func list) (fun_env : (t list * t) Idmap.t)
  ...
  | f::funcs ->
    if Idmap.mem f.fun_name fun_env then
      raise_error "semantic" f.loc ("duplicate function definition " ^ show_id f.fun_name)
    else
