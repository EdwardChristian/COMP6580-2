...
(* Convert an atomic source expression *)
let exp_to_atomic (e : S.exp) : atomic_exp =
  match e with
  | S.Ident (id, []) -> Ident (id_to_var id)
  | S.Num n -> Num n
  | S.Bool b -> bool_to_num b
  | S.Ident (_, _::_) | S.Op _ | S.Uop _ | S.Lop _ | S.Array _ | S.List _ | S.Call _ ->
    raise (InternalError "non-flat expression in blockStructure")

let tmp_var = NamedTmp("BS", 0)

let makeList (v : var) (hd : S.exp) (tl : S.exp list) : block_elem list = 
  match (hd,tl) with 
  | (hd',[]) -> [Call (Some v, "makeList", [exp_to_atomic hd'])] (* This creates a list with an empty tail *)
  | (hd',tl') -> [Call (Some v, "cons", [exp_to_atomic hd'] @ [Num (Int64.of_int (List.length tl'))] @ List.map exp_to_atomic tl')] (* if there are elements in the tail then cons them *)
...
let flat_e_to_assign (x : S.id) (e : S.exp) : block_elem list =
  ...
  | S.Op (hd, Tokens.Cons, S.List (l_hd, l_tl) ) -> 
    print_endline "Camels are bae";makeList v hd ([l_hd] @ l_tl)
  | S.Op (ae1, op, ae2) ->
    print_endline "Camels are bae";[AssignOp (v, exp_to_atomic ae1, op, exp_to_atomic ae2)]
  | S.Uop (Tokens.Not, ae) ->
    (* !x == (x = false) *)
    [AssignOp (v, exp_to_atomic ae, Tokens.Eq, Num 0L)]
  | S.Lop (S.Ident(id, []), lop) -> 
    [Call (Some v, Tokens.show_lop lop, [Ident (id_to_var id)])]
  | S.Lop (_, _) -> raise (InternalError "non-identifier parsed with a list operator in blockStructure")
  | S.List (hd, tl) ->
    makeList v hd tl
  | S.Array es ->
...
let flat_exp_to_test (e : S.exp) : test =
  ...
  | S.Lop _ ->
    raise (InternalError "lop test in blockStructure")
  | S.List _ ->
    raise (InternalError "list alloc test in blockStructure")
  | S.Array _ ->
