...
let rec might_have_effect (e : exp) : bool =
  ...
  | Lop (e, _) -> might_have_effect e
  | Uop (_, e) -> might_have_effect e
  | List (hd, tl) -> might_have_effect hd || List.exists might_have_effect tl (* same approach as an array but look at both head and tail *)
...
let rec fold_exp (env : exp Idmap.t) (e : exp) : exp =
  ...
  | Uop (uop, e) ->
    ...
    (match (uop, o) with
     ...
     | _ -> Uop (uop, o))
  | Lop (e, lop) -> 
    Lop (fold_exp env e, lop)
  | List (hd, tl) ->
    List (hd, (List.map (fold_exp env) tl))
  | Array es ->
