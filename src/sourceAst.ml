...
type exp =
  ...
  | Array of exp list
  (* Create a list made up of a head and a tail. The head is an expression, the tail is a list of expressions *)
  | List of exp * exp list
...
let rec pp_exp (fmt : F.formatter) (exp : exp) : unit =
  ...
  | Array es ->
    F.fprintf fmt "@[<2>array@ %a@]"
      (pp_array_list pp_exp) es
  | List (hd, tl) ->     
    F.fprintf fmt "@[<2>list@ %a@ %a@]"
      pp_exp hd (pp_array_list pp_exp) tl
...
let rec parse_atomic_exp (toks : T.tok_loc list) : exp * T.tok_loc list =
  ...
  | (T.Ident i, _) :: (T.Lop lop, _) ::toks ->
    (Lop (Ident(Source (i,None), []), lop), toks)  
  | (T.Ident i, _) :: toks ->
  ...
  | (T.Array, _) :: toks ->
    ...
    (Array indices, toks)
  | (T.List, _) :: toks ->
    let (hd, tl, toks) = parse_head_tail toks in
    (List (hd, tl), toks)
  | (T.Lparen, _) :: toks ->
...
and parse_head_tail (toks : T.tok_loc list) : exp * exp list * T.tok_loc list =
  match toks with
  | (T.Lbrac, _) :: (T.Rbrac, _) :: toks -> (Num 0L, [], toks) (* check for empty list and the head of an empty list is the number 0 *)
  | (T.Lbrac, _) :: toks ->
    (match parse_exp toks with
    | (hd, (T.Rbrac, _) :: toks) -> (hd, [], toks)
    | (hd, (T.Comma, _) :: toks) -> let (tl,toks) = parse_tail toks [] in (hd,tl,toks)
    | _ -> eof_error "a list head related token (exp, rbrac or comma)"
    )
  | (t, ln) :: _ -> parse_error_expect ln t T.Lbrac "a left bracket"
  | [] -> eof_error "a list related token"
and parse_tail (toks : T.tok_loc list) (acc : exp list) : exp list * T.tok_loc list = 
  match parse_exp toks with
  | (e, (T.Rbrac, _) :: toks) -> (acc @ [e] ,toks) (* Append can reduce performance for very very large lists *)
  | (e, (T.Comma, _) :: toks) -> parse_tail toks ([e] @ acc) (* Append to acc to keep history *)
  | _ -> eof_error "a list tail related token (exp, rbrac or comma)"
