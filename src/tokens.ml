...
(* list operations, may add list size *)
let show_lop lop = 
  match lop with 
  | Head -> "hd" (* head of list *)
  | Tail -> "tl" (* tail of list *)
...
type token =
  ...
  | Op of op
  | Uop of uop
  | Lop of lop (* list found, so expect list operations *)
  (* Punctuation *)
  | Lparen
  | Rparen
  | Lcurly
...
let keywords : (string * token) list =
  ...
  List.map (fun o -> (show_op o, Op o))
    [Plus; Minus; Times; Div; Lt; Gt; Eq; And; Or; Lshift; BitOr; BitAnd; Cons] @
  List.map (fun t -> (show_token t, t))
    [Do; While; If; Then; Else; Array; List; Assign; True; Input; Output; False;
     Lparen; Rparen; Lcurly; Rcurly; Lbrac; Rbrac; Int; Bool; Colon; Let;
     Return; Function; Comma] @
  List.map (fun o -> (show_lop o, Lop o)) (* map list key * string to token e.g. ('hd', Head) *)
    [Empty; Head; Tail]
