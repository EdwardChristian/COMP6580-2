...
let is_flat (e : exp) : bool =
  ...
  | Uop (_, e) -> is_atomic e
  | Lop (e, _) -> is_atomic e
  | List (hd, tl) -> is_atomic hd && List.for_all is_atomic tl (* same approach as for Array but again, including the head *)
...
let unnest (stmts : stmt list) : stmt list =
  ...
  let rec unnest_exp (e : exp) : stmt list * exp =
    ...
    | Lop (e, lop) -> 
      let (s, a) = unnest_exp_atomic e in 
      (s, Lop (a, lop))
    | List (hd, tl) -> (* similar approach to array, but also unnest the HEAD and append to list, u stands for unnested *)
      let (s_hd_list, uhd) = unnest_exp_atomic hd in
      let (s_tl_list, utl) = List.split (List.map unnest_exp_atomic tl) in 
      (s_hd_list @ List.flatten s_tl_list, List (uhd, utl))
    | Array es ->
      let (s_list, aes) = List.split (List.map unnest_exp_atomic es) in
